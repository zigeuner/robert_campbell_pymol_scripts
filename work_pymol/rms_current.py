#! /usr/bin/env python3
# Copyright (c) 2005 Robert L. Campbell (rlc1@queensu.ca)
...
DESCRIPTION
  ...
  "rms_current" allows the calculation of the RMSD for object1 on
  object2 using the atoms in selection1 and selection2 (side chains
  are ignored) without doing any fitting first.  The residue names,
  residue numbers chain identifiers, segment identifiers, and alt ids
  of selection1 are changed to match those in selection2, temporarily.
  This allows the normal "rms_cur" command to work.  They are reset
  after "rms_cur" is run and two new selections are created showing the
  selected atoms.
...
EXAMPLES
  ...
  m_count=cmd.count_atoms("%s & %s" % (obj1,select1))

  n=cmd.get_model("%s & %s" % (obj2,select2))
  n_count=cmd.count_atoms("%s & %s" % (obj2,select2))
...
# change the segid for obj2 and select2
  ...
  print("Altered to:")
  print("%s & %s & segi %s\n" % (obj1,select2,seg_rms),end=" ")
  print("%s & %s & segi %s\n" % (obj2,select2,seg_rms),end=" ")
  print("--------------------------------------------\n")
...
# reset segi of obj2 & select2 from stored list
  ...
  print("For %d atoms in %s and %s to %d atoms in %s and %s" % (m_count,obj1,select1,n_count,obj2,select2))
