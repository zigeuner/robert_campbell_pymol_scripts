#! /usr/bin/env python3
# Copyright (c) 2003 Robert L. Campbell (rlc1@queensu.ca)
...
def stride_extract(pdb_file):
  ...
  for line in stride_output:
    ...
    if words[0] == "ASG":
      ...
      sec_short = words[5]
      sec_long = words[6]
# replace the long version using the .setdefault dictionary method
#      if chain in secondary:
#        secondary[chain][resnum] = sec_short
#      else:
#        secondary[chain] = {resnum:sec_short}
      secondary.setdefault(chain,{resnum:sec_short})[resnum] = sec_short
      residue.setdefault(chain,{resnum:resname})[resnum] = resname
...
def stride2pymol(mol='',sel=''):
  ...
  chain_list = list(sec_dict.keys())
  chain_list.sort()
  for chain in chain_list:
    res_list = list(sec_dict[chain].keys())
    res_list.sort()
